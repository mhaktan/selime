...
export const StationList: React.FC = () => {
  const list = useListQuery<StationRecord>({ resource: 'Station' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<StationRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<StationRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('Station', (ids) => { ids.forEach(id => triggerFlows('delete', 'Station', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as StationRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create Station" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <StationCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <StationEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
