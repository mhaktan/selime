...
interface AircraftTypeEditProps {
  record: AircraftTypeRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const AircraftTypeEdit: React.FC<AircraftTypeEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<AircraftTypeRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<AircraftTypeRecord>) =>
      dataProvider.update('AircraftType', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'AircraftType', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit AircraftType</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  <TkInput mode="text" label="Type Code *" value={String(form.typeCode ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('typeCode', v))(e.detail)} />
                </div>
                <div>
                ...
                <div>
                  <TkInput mode="number" label="Seat Capacity" value={String(form.seatCapacity ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('seatCapacity', Number(v)))(e.detail)} />
                </div>
            </div>
          </div>
