...
import { SnagReportPartCreate } from './SnagReportPartCreate';
import { SnagReportPartEdit } from './SnagReportPartEdit';
...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'serialNumber', header: 'Serial Number', sortable: true, searchable: true, filterType: 'text' },
  { field: 'quantityUsed', header: 'Quantity Used', sortable: true },
  { field: 'snagReportId', header: 'Snag Report', sortable: true },
  { field: 'partCatalogId', header: 'Part Catalog', sortable: true },
];
...
export const SnagReportPartList: React.FC = () => {
  const list = useListQuery<SnagReportPartRecord>({ resource: 'SnagReportPart' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<SnagReportPartRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<SnagReportPartRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('SnagReportPart', (ids) => { ids.forEach(id => triggerFlows('delete', 'SnagReportPart', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as SnagReportPartRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create SnagReportPart" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <SnagReportPartCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <SnagReportPartEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
