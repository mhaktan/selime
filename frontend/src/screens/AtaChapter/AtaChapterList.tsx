...
import { AtaChapterCreate } from './AtaChapterCreate';
import { AtaChapterEdit } from './AtaChapterEdit';
...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'ataNumber', header: 'Ata Number', sortable: true, searchable: true, filterType: 'text' },
...
export const AtaChapterList: React.FC = () => {
  const list = useListQuery<AtaChapterRecord>({ resource: 'AtaChapter' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<AtaChapterRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<AtaChapterRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('AtaChapter', (ids) => { ids.forEach(id => triggerFlows('delete', 'AtaChapter', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as AtaChapterRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create AtaChapter" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <AtaChapterCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <AtaChapterEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
